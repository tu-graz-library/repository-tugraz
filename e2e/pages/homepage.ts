...
export class HomePage {
   ...
   * using the actual language link found in debug /lang/de
   * using other language selectors as fallback */
  async changeLanguageToGerman(): Promise<void> {
    try {
      ...
      const germanLinkFallback = this.page.locator(
        'a:has-text("DE"), a:has-text("Deutsch")'
      );
   ...
   * Navigate to base URL - default to english */
  async changeLanguageToEnglish(): Promise<void> {
    try {
      ...
      const englishLinkFallback = this.page.locator(
        'a:has-text("EN"), a:has-text("English")'
      );
